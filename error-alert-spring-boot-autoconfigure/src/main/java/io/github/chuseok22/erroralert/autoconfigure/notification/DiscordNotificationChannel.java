...
/**
 * Discord Webhook으로 알림을 전송한다. 메시지가 Discord 제한(2000자)을 초과하면 청크로 분할한다.
 */
public class DiscordNotificationChannel implements NotificationChannel {
    ...
    public DiscordNotificationChannel(String webhookUrl) {
    ...
    private void sendChunk(String content) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                log.warn("Discord 전송 실패 (status={})", response.statusCode());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log.warn("Discord 전송 인터럽트", e);
        } catch (Exception e) {
            log.warn("Discord 전송 오류", e);
        }
    }
