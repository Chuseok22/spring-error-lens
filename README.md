...
현재 Nexus 사설 저장소를 통해 배포됩니다. 아래와 같이 저장소와 의존성을 추가하세요.
...
repositories {
    maven {
        url 'https://your-nexus-host/repository/maven-releases/'
        credentials {
...
dependencies {
    implementation 'io.github.chuseok22:error-alert-spring-boot-starter:0.1.0'
}
```
...
<repositories>
  <repository>
    <id>nexus</id>
    <url>https://your-nexus-host/repository/maven-releases/</url>
  </repository>
</repositories>

<dependency>
  <groupId>io.github.chuseok22</groupId>
...
`NotificationChannel` 인터페이스를 구현하고 Spring Bean으로 등록하면 됩니다.
...
`NotificationDispatcher`는 컨텍스트에 등록된 모든 `NotificationChannel` Bean을 자동으로 수집하여 사용합니다.
